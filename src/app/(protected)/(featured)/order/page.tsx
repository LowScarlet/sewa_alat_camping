...
export default function Order() {
  ...
  const deleteItem = async (orderId: number) => {
    try {
      await axiosInstance.post(`/api/client/customer/order/delete/${orderId}`);
      mutate((key: string) => key.startsWith('/api/client/customer/order/'));
    } catch (err: any) {
      console.error('Error adding to cart:', err);
    }
  };

  if (!isAuthenticated || !authUser || !dataOrder) {
    return (<>Error</>)
  }
